...
Данный проект является продолжением и расширяет REST API механизмами автоматического документирования. Реализованы:
...
Тестирование защищённых эндпоинтов
    ...
    Выполнить POST /auth/login прямо в Swagger UI – куки access_token и refresh_token сохранятся автоматически (браузер).

    После этого вызываем защищённые методы (/auth/whoami, /items/ и др.) – они будут выполняться от имени авторизованного пользователя.
